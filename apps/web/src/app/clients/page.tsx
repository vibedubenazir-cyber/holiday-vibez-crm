...
export default function ClientsPage() {
  ...
  const canManageLoyalty = me?.role === Role.ADMIN || me?.role === Role.DIRECTOR || me?.role === Role.BRANCH_MANAGER;

  const [expandedLoyalty, setExpandedLoyalty] = useState<string | null>(null);
  const [loyalty, setLoyalty] = useState<Record<string, LoyaltyAccountDTO>>({});
  ...
  async function loadLoyalty(clientId: string) {
    try {
      setLoyalty((prev) => ({ ...prev, [clientId]: prev[clientId] }));
      const account = await api.get<LoyaltyAccountDTO>(`/loyalty/${clientId}`);
      setLoyalty((prev) => ({ ...prev, [clientId]: account }));
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Failed to load loyalty account');
    }
  }

  function toggleLoyalty(clientId: string) {
    const next = expandedLoyalty === clientId ? null : clientId;
    setExpandedLoyalty(next);
    setPointsForm({ points: '', description: '' });
    if (next) loadLoyalty(next);
  }

  async function handleEarn(clientId: string) {
    try {
      await api.post(`/loyalty/${clientId}/earn`, { points: Number(pointsForm.points), description: pointsForm.description || undefined });
      setPointsForm({ points: '', description: '' });
      await loadLoyalty(clientId);
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Failed to award points');
    }
  }

  async function handleRedeem(clientId: string) {
    try {
      await api.post(`/loyalty/${clientId}/redeem`, { points: Number(pointsForm.points), description: pointsForm.description || undefined });
      setPointsForm({ points: '', description: '' });
      await loadLoyalty(clientId);
    } catch (err) {
  ...
  async function handleReferralBonus(clientId: string) {
    try {
      await api.post(`/loyalty/${clientId}/referral-bonus`, { points: Number(pointsForm.points), description: pointsForm.description || undefined });
      setPointsForm({ points: '', description: '' });
      await loadLoyalty(clientId);
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Failed to award referral bonus');
    }
  }
      ...
      <div className="mt-4 overflow-hidden bg-white">
        <table className="w-full text-sm">
          ...
          <tbody>
              ...
              return (
                <Fragment key={c.id}>
                  <tr className="border-t border-slate-100">
                    ...
                    <td className="px-4 py-2">
                      <button onClick={() => toggleLoyalty(c.id)} className="text-brand hover:underline">
                        {expandedLoyalty === c.id ? 'Hide' : 'View'}
                      </button>
                    </td>
                    {canManage && (
            ...
            {clients.length === 0 && (
              <tr><td colSpan={6} className="px-4 py-6 text-center text-slate-400">No clients yet.</td></tr>
            )}
          </tbody>
        </table>
      </div>
