  ...
  return (
    <AppShell>
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-semibold text-slate-800">Website CMS</h1>
        {canManage && (
          <div className="flex gap-2">
            <button onClick={() => setShowSettings((s) => !s)} className="rounded-md border border-slate-300 px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100">
      ...
      {!canManage && <p className="mt-1 text-xs text-slate-400">Read-only — only Admin/Director can manage CMS content.</p>}
      ...
      {canManage && showSettings && (
        <div className="mt-4 rounded-lg border border-slate-200 bg-white p-4">
          ...
          <form onSubmit={handleSaveSetting} className="mt-3 flex gap-2">
            <input required placeholder="Key (e.g. contact_email)" value={settingForm.key} onChange={(e) => setSettingForm({ ...settingForm, key: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
            <input required placeholder="Value" value={settingForm.value} onChange={(e) => setSettingForm({ ...settingForm, value: e.target.value })} className="flex-1 rounded-md border border-slate-300 px-3 py-2 text-sm" />
      ...
      <div className="mt-4 flex gap-1 rounded-md border border-slate-200 bg-white p-1 text-sm">
        {TYPE_TABS.map((t) => (
          <button
            key={t}
            onClick={() => setType(t)}
            className={`rounded px-3 py-1.5 font-medium ${type === t ? 'bg-brand text-white' : 'text-slate-600'}`}
          >
            {TYPE_LABELS[t]}
          </button>
        ))}
      ...
      {canManage && showForm && (
        <form onSubmit={handleCreate} className="mt-4 grid grid-cols-1 gap-3 rounded-lg border border-slate-200 bg-white p-4 sm:grid-cols-2">
          <input required placeholder="Title" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm sm:col-span-2" />
          <input placeholder="Subtitle" value={form.subtitle} onChange={(e) => setForm({ ...form, subtitle: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm sm:col-span-2" />
          <input placeholder="Image URL" value={form.imageUrl} onChange={(e) => setForm({ ...form, imageUrl: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          <input placeholder="Link URL" value={form.linkUrl} onChange={(e) => setForm({ ...form, linkUrl: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          {type === CmsContentType.TESTIMONIAL && (
            <input type="number" min={1} max={5} placeholder="Rating (1-5)" value={form.rating} onChange={(e) => setForm({ ...form, rating: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          )}
          <textarea placeholder="Body / quote text" value={form.body} onChange={(e) => setForm({ ...form, body: e.target.value })} rows={3} className="rounded-md border border-slate-300 px-3 py-2 text-sm sm:col-span-2" />
      ...
      <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {content.map((item) => (
          <div key={item.id} className="rounded-lg border border-slate-200 bg-white p-4">
            <div className="flex items-start justify-between">
              ...
              <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${item.active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600'}`}>
                {item.active ? 'Active' : 'Inactive'}
              </span>
            </div>
            {item.subtitle && <p className="mt-1 text-xs text-slate-500">{item.subtitle}</p>}
            {item.rating && <p className="mt-1 text-sm text-amber-500">{'★'.repeat(item.rating)}</p>}
        ...
        {content.length === 0 && <p className="text-sm text-slate-400">No {TYPE_LABELS[type].toLowerCase()} yet.</p>}
      </div>
    </AppShell>
  );
}
