  ...
  return (
    <AppShell>
      ...
      <div className="mt-4 flex items-center gap-3">
        ...
        <select value={branchId} onChange={(e) => setBranchId(e.target.value)} className="rounded-lg border border-slate-300 dark:border-slate-600 px-3 py-2 text-sm focus:border-brand focus:outline-none focus:ring-2 focus:ring-brand-100 dark:focus:ring-brand-900/40 transition-colors">
      ...
      {canSetTargets && (
        <form onSubmit={handleCreate} className="mt-4 grid grid-cols-1 gap-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-card transition-shadow hover:shadow-card-hover p-4 sm:grid-cols-2 lg:grid-cols-4">
          <select value={form.scope} onChange={(e) => setForm({ ...form, scope: e.target.value as 'CONSULTANT' | 'BRANCH', scopeId: '' })} className="rounded-lg border border-slate-300 dark:border-slate-600 px-3 py-2 text-sm focus:border-brand focus:outline-none focus:ring-2 focus:ring-brand-100 dark:focus:ring-brand-900/40 transition-colors">
            <option value="CONSULTANT">Consultant target</option>
            <option value="BRANCH">Branch target</option>
          </select>
          {form.scope === 'CONSULTANT' ? (
            <select value={form.scopeId} onChange={(e) => setForm({ ...form, scopeId: e.target.value })} className="rounded-lg border border-slate-300 dark:border-slate-600 px-3 py-2 text-sm focus:border-brand focus:outline-none focus:ring-2 focus:ring-brand-100 dark:focus:ring-brand-900/40 transition-colors">
          ...
          <select value={form.period} onChange={(e) => setForm({ ...form, period: e.target.value })} className="rounded-lg border border-slate-300 dark:border-slate-600 px-3 py-2 text-sm focus:border-brand focus:outline-none focus:ring-2 focus:ring-brand-100 dark:focus:ring-brand-900/40 transition-colors">
            <option value="WEEK">Weekly</option>
            <option value="MONTH">Monthly</option>
            <option value="QUARTER">Quarterly</option>
          </select>
          <input required type="number" placeholder="Revenue target (₹)" value={form.revenueTarget} onChange={(e) => setForm({ ...form, revenueTarget: e.target.value })} className="rounded-lg border border-slate-300 dark:border-slate-600 px-3 py-2 text-sm focus:border-brand focus:outline-none focus:ring-2 focus:ring-brand-100 dark:focus:ring-brand-900/40 transition-colors" />
      ...
      <div className="mt-2 overflow-hidden rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-card transition-shadow hover:shadow-card-hover">
        <table className="w-full text-sm">
          ...
          <tbody>
            {leaderboard.map((row) => (
              <tr key={row.consultantId} className="border-t border-slate-100 dark:border-slate-800">
                ...
                <td className="px-4 py-2">
                  <span className={row.conversionPct !== undefined && row.conversionPct < 40 ? 'text-red-600 dark:text-red-400 font-medium' : 'text-slate-700 dark:text-slate-200'}>
                    {row.conversionPct ?? 0}%
                  </span>
                </td>
              </tr>
            ))}
            {leaderboard.length === 0 && <tr><td colSpan={4} className="px-4 py-6 text-center text-slate-400 dark:text-slate-500">No consultants or targets for this branch yet.</td></tr>}
          </tbody>
        </table>
      </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {companyLeaderboard.map((row) => (
                  <tr key={row.branchId} className="border-t border-slate-100 dark:border-slate-800">
                    <td className="px-4 py-2">{row.name}</td>
                    <td className="px-4 py-2">₹{row.revenueTarget.toLocaleString('en-IN')}</td>
                    <td className="px-4 py-2">₹{row.revenueAchieved.toLocaleString('en-IN')}</td>
                  </tr>
