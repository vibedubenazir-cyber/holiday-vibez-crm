...
export default function TargetsPage() {
  ...
  return (
    <AppShell>
      <h1 className="text-lg font-semibold text-slate-800">Targets & Leaderboard</h1>
      {error && <p className="mt-3 text-sm text-red-600">{error}</p>}

      <div className="mt-4 flex items-center gap-3">
        <label className="text-sm text-slate-600">Branch:</label>
        <select value={branchId} onChange={(e) => setBranchId(e.target.value)} className="rounded-md border border-slate-300 px-3 py-2 text-sm">
      ...
      {canSetTargets && (
        <form onSubmit={handleCreate} className="mt-4 grid grid-cols-1 gap-3 rounded-lg border border-slate-200 bg-white p-4 sm:grid-cols-2 lg:grid-cols-4">
          <select value={form.scope} onChange={(e) => setForm({ ...form, scope: e.target.value as 'CONSULTANT' | 'BRANCH', scopeId: '' })} className="rounded-md border border-slate-300 px-3 py-2 text-sm">
            <option value="CONSULTANT">Consultant target</option>
            <option value="BRANCH">Branch target</option>
          </select>
          {form.scope === 'CONSULTANT' ? (
            <select value={form.scopeId} onChange={(e) => setForm({ ...form, scopeId: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm">
              <option value="">Select consultant</option>
              {branchConsultants.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
          ) : (
            <input value={branchId} readOnly className="rounded-md border border-slate-300 bg-slate-50 px-3 py-2 text-sm text-slate-500" />
          )}
          <select value={form.period} onChange={(e) => setForm({ ...form, period: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm">
            <option value="WEEK">Weekly</option>
            <option value="MONTH">Monthly</option>
            <option value="QUARTER">Quarterly</option>
          </select>
          <input required type="number" placeholder="Revenue target (₹)" value={form.revenueTarget} onChange={(e) => setForm({ ...form, revenueTarget: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
