...
export default function StoragePage() {
  ...
  return (
    <AppShell>
      <h1 className="text-lg font-semibold text-slate-800">File Storage</h1>
      ...
      <div className="mt-4 max-w-xl rounded-lg border border-slate-200 bg-white p-5">
        <input
          ...
          disabled={uploading}
          className="block w-full text-sm text-slate-600 file:mr-4 file:rounded-md file:border-0 file:bg-brand file:px-3 file:py-2 file:text-sm file:font-medium file:text-white hover:file:bg-brand-dark"
        />
        ...
        {uploaded && (
          <div className="mt-4 space-y-3 border-t border-slate-100 pt-4">
            {/^\.(png|jpg|jpeg|gif|webp)$/i.test(uploaded.url.slice(uploaded.url.lastIndexOf('.'))) && (
              <img src={absoluteUrl(uploaded.url)} alt={uploaded.originalName} className="max-h-48 rounded-md border border-slate-200" />
            )}
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-full rounded-md border border-slate-300 bg-slate-50 px-3 py-2 text-xs text-slate-600"
              />
              <button
                onClick={handleCopy}
                className="shrink-0 rounded-md border border-slate-200 px-3 py-2 text-sm text-slate-600 hover:bg-slate-100"
              >
