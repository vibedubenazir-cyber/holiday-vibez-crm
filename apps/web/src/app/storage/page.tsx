...
export default function StoragePage() {
  ...
  return (
    <AppShell>
      <h1 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white">File Storage</h1>
      ...
      <div className="mt-4 max-w-xl rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-card transition-shadow hover:shadow-card-hover p-5">
        <input
          type="file"
          accept=".png,.jpg,.jpeg,.gif,.webp,.pdf"
          onChange={handleFileChange}
          disabled={uploading}
          className="block w-full text-sm text-slate-600 dark:text-slate-300 file:mr-4 file:rounded-lg file:border-0 file:bg-brand file:px-3 file:py-2 file:text-sm file:font-medium file:text-white hover:file:bg-brand-dark"
        />
        ...
        {uploaded && (
          <div className="mt-4 space-y-3 border-t border-slate-100 dark:border-slate-800 pt-4">
            {/^\.(png|jpg|jpeg|gif|webp)$/i.test(uploaded.url.slice(uploaded.url.lastIndexOf('.'))) && (
              <img src={absoluteUrl(uploaded.url)} alt={uploaded.originalName} className="max-h-48 rounded-lg border border-slate-200 dark:border-slate-700" />
            )}
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 px-3 py-2 text-xs text-slate-600 dark:text-slate-300"
              />
              <button
                onClick={handleCopy}
