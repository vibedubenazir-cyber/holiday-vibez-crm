  ...
  return (
    <AppShell>
      ...
      <div className="mt-4 space-y-3">
                  ...
                  <tbody>
                    {(p.items ?? []).map((item) => (
                      <tr key={item.id} className="border-t border-slate-100 dark:border-slate-800">
                        ...
                        <td className="py-1 text-slate-500 dark:text-slate-400">{item.rateCard?.name}</td>
                        <td className="py-1">{item.quantity}</td>
                      </tr>
                    ))}
                    {(!p.items || p.items.length === 0) && (
                      <tr><td colSpan={4} className="py-2 text-center text-slate-400 dark:text-slate-500">No items yet.</td></tr>
                    )}
                  </tbody>
                </table>
        ...
        {packages.length === 0 && <p className="text-sm text-slate-400 dark:text-slate-500">No packages yet.</p>}
      </div>
    </AppShell>
  );
}
