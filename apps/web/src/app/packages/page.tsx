    ...
    <AppShell>
      ...
      {canManage && showForm && (
        <form onSubmit={handleCreate} className="mt-4 grid grid-cols-1 gap-3 rounded-lg border border-slate-200 bg-white p-4 sm:grid-cols-2 lg:grid-cols-3">
          <input required placeholder="Package name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          ...
          <input placeholder="Theme (e.g. Honeymoon)" value={form.theme} onChange={(e) => setForm({ ...form, theme: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          <input required type="number" min={1} placeholder="Duration (days)" value={form.durationDays} onChange={(e) => setForm({ ...form, durationDays: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          <input type="number" placeholder="Base price (indicative)" value={form.basePrice} onChange={(e) => setForm({ ...form, basePrice: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
              ...
              <div className="mt-3 border-t border-slate-100 pt-3">
                ...
                {canManage && (
                  <div className="mt-3 flex flex-wrap items-end gap-2">
                    <select value={itemForm.rateCardId} onChange={(e) => setItemForm({ ...itemForm, rateCardId: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm">
                      <option value="">Rate card...</option>
                      {rates.map((r) => <option key={r.id} value={r.id}>{r.name} ({r.destination})</option>)}
                    </select>
                    <input type="number" min={1} placeholder="Day #" value={itemForm.dayNumber} onChange={(e) => setItemForm({ ...itemForm, dayNumber: e.target.value })} className="w-20 rounded-md border border-slate-300 px-3 py-2 text-sm" />
                    <input placeholder="Description" value={itemForm.description} onChange={(e) => setItemForm({ ...itemForm, description: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
                    <input type="number" min={1} placeholder="Qty" value={itemForm.quantity} onChange={(e) => setItemForm({ ...itemForm, quantity: e.target.value })} className="w-16 rounded-md border border-slate-300 px-3 py-2 text-sm" />
