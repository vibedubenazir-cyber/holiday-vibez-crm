  ...
  return (
    <AppShell>
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-extrabold tracking-tight text-brand-700">Currency Exchange</h1>
        {canManage && (
          ...
          <tbody>
            {rates.map((r) => (
              <tr key={r.id} className="border-t border-slate-100">
                ...
                <td className="px-4 py-2">
                  <span className={`rounded-lg px-2 py-0.5 text-xs font-medium ${r.source === 'API' ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-600'}`}>
                    {r.source}
                  </span>
                </td>
