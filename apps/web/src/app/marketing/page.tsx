  ...
  return (
    <AppShell>
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-semibold text-slate-800">Marketing</h1>
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          <p className="text-xs uppercase tracking-wide text-slate-400">Campaigns sent this month</p>
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-4 sm:col-span-2">
          <p className="text-xs uppercase tracking-wide text-slate-400">Leads by source</p>
          <div className="mt-2 flex flex-wrap gap-2">
            {dashboard?.leadsBySource.map((s) => (
              <span key={s.source} className="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-600">
                {s.source}: {s.count}
              </span>
            ))}
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-4 sm:col-span-3">
          <p className="text-xs uppercase tracking-wide text-slate-400">Upcoming birthdays &amp; anniversaries (next 7 days)</p>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="mt-4 grid grid-cols-1 gap-3 rounded-lg border border-slate-200 bg-white p-4 sm:grid-cols-2 lg:grid-cols-4">
          <input required placeholder="Campaign name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          ...
          <select value={form.audienceBranchId} onChange={(e) => setForm({ ...form, audienceBranchId: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm">
            ...
            {campaigns.map((c) => (
              <tr key={c.id} className="border-t border-slate-100">
                ...
                <td className="px-4 py-2">
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${c.status === 'SENT' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600'}`}>
                    {c.status}
                  </span>
                </td>
